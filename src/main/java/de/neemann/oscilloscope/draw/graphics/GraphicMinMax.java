...
import de.neemann.oscilloscope.draw.graphics.text.formatter.GraphicsFormatter;

import java.awt.font.FontRenderContext;
import java.awt.geom.Rectangle2D;

import static de.neemann.oscilloscope.draw.graphics.GraphicSwing.getMirrorYOrientation;
...
public class GraphicMinMax extends Graphic {
    ...
    /**
     * Returns true if the given position is inside of the bounding box
     *
     * @param pos the position
     * @return true if inside
     */
    public boolean match(VectorInterface pos) {
        float x = pos.getXFloat();
        float y = pos.getYFloat();
        return x >= min.getXFloat() && x <= max.getXFloat()
                && y >= min.getYFloat() && y <= max.getYFloat();
    }
