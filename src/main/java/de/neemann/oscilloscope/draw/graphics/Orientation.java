...
public enum Orientation {
    ...
    /**
     * Creates a orientation from the given direction
     *
     * @param dir the direction
     * @return the matching orientation
     */
    public static Orientation from(VectorInterface dir) {
        VectorFloat n = dir.norm();

        int x = getBy(n.getXFloat());
        int y = 2 - getBy(n.getYFloat());
    ...
    private static int getBy(float f) {
        if (f < -0.5)
            return 2;
        else if (f > 0.5)
            return 0;
